...
export default async function suite() {
  describe('Check project loan approval', async () => {
    ...
    it('when approving a project loan', async function () {
      ...
      expect(daoApprovalRequest.approvalsProvided.toString()).to.be.equal(BigNumber.from(1).toString());
      expect(daoApprovalRequest.isApproved).to.be.equal(true);

      const isPaused = await this.fundingNFTContract.transfersPaused(this.loanId.toString());
      const loanStatus = await this.registryContract.loanStatus(this.loanId.toString());
