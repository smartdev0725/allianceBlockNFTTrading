...
export default async function suite() {
  describe('Personal loan requests', async () => {
    it('when requesting an interest only personal loan', async function () {
      ...
      let initSeekerCollateralBalance = await this.collateralTokenContract.balanceOf(this.seeker);
      let initEscrowCollateralBalance = await this.collateralTokenContract.balanceOf(this.escrowContract.address);
      let initEscrowFundingNftBalance = await this.fundingNFTContract.balanceOf(this.escrowContract.address, loanId);
      ...
      const tx = await this.registryContract.connect(this.seekerSigner).requestPersonalLoan(
        amountRequested,
      ...
      const newSeekerCollateralBalance = await this.collateralTokenContract.balanceOf(this.seeker);
      const newEscrowCollateralBalance = await this.collateralTokenContract.balanceOf(this.escrowContract.address);
      const newEscrowFundingNftBalance = await this.fundingNFTContract.balanceOf(this.escrowContract.address, loanId);
      ...
      const loanPayments = await this.registryContract.personalLoanPayments(loanId);
      ...
      expect(loanDetails.collateralToken.toString()).to.be.equal(this.collateralTokenContract.address);
    ...
    it('when requesting a nominal plus interest personal loan', async function () {
      ...
      const newSeekerCollateralBalance = await this.collateralTokenContract.balanceOf(this.seeker);
      const newEscrowCollateralBalance = await this.collateralTokenContract.balanceOf(this.escrowContract.address);
      const newEscrowFundingNftBalance = await this.fundingNFTContract.balanceOf(this.escrowContract.address, this.loanId);
      ...
      // Correct Event.
      expectEvent(tx.receipt, 'PersonalLoanRequested', {loanId: this.loanId, user: this.seeker, amount: amountRequested.toString()});
