  ...
  describe('Check execute lottery', async () => {
    ...
    context('When doing various withdrawals scenarios', async function () {
      ...
      it('When withdrawing with 0 ticketsToLock', async function () {
        ...
        const balanceOfProjectTokensBefore =
          await this.projectTokenContract.balanceOf(this.lender1);
        ...
        const balanceOfProjectTokensAfter =
          await this.projectTokenContract.balanceOf(this.lender1);

        const projectTokensGot = balanceOfProjectTokensAfter.sub(
          balanceOfProjectTokensBefore
        );
        const projectTokensToGet = ticketsToWin.mul(tokensPerTicket);

        // Then
        expect(projectTokensGot.toString()).to.be.equal(
          projectTokensToGet.toString()
        );
      });
