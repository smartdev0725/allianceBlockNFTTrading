...
export default async function suite() {
  describe('Show investment interest', async () => {
    it('reverts when investment is not approved yet', async function () {
      ...
      const deadline1 = 1630458579;
      const deadline2 = 1630558579;
    ...
    it('increments the number of purchased partitions', async function () {
      ...
      const investmentDetailsBefore = await this.investmentWithMilestoneContract.investmentMilestoneDetails(
        this.projectId
      );
      ...
      // Then
      const investmentDetailsAfter = await this.investmentWithMilestoneContract.investmentMilestoneDetails(
        this.projectId
      );
    ...
    it('when showing interest and is eligible for immediate tickets', async function () {
      ...
      const ticketsRemainingBefore = await this.investmentWithMilestoneContract.ticketsRemaining(this.projectId);
      ...
      // Then
      const ticketsRemainingAfter = await this.investmentWithMilestoneContract.ticketsRemaining(this.projectId);
      const ticketsWon = await this.investmentWithMilestoneContract.ticketsWonPerAddress(this.projectId, this.lender1);
    ...
    it('should give tickets for reputational ALBT', async function () {
      ...
      const ticketsRemainingBefore =
        await this.investmentWithMilestoneContract.ticketsRemaining(this.projectId);
      const totalLotteryNumbersPerInvestmentBefore =
        await this.investmentWithMilestoneContract.totalLotteryNumbersPerInvestment(
          this.projectId
        );
      const remainingTicketsPerAddressBefore =
        await this.investmentWithMilestoneContract.remainingTicketsPerAddress(
          this.projectId,
          this.lender1
        );
      const ticketsWonPerAddressBefore =
        await this.investmentWithMilestoneContract.ticketsWonPerAddress(
          this.projectId,
          this.lender1
        );
    ...
    it('when all tickets are provided as immediate (no lottery)', async function () {
      ...
      const ticketsRemainingBefore = await this.investmentWithMilestoneContract.ticketsRemaining(this.projectId.add(1));
      ...
      // Then
      const ticketsRemainingAfter = await this.investmentWithMilestoneContract.ticketsRemaining(this.projectId.add(1));
      const ticketsWonForLender1 = await this.investmentWithMilestoneContract.ticketsWonPerAddress(this.projectId.add(1), this.lender1);
      const ticketsWonForLender2 = await this.investmentWithMilestoneContract.ticketsWonPerAddress(this.projectId.add(1), this.lender2);
      const status = await this.investmentWithMilestoneContract.projectStatus(this.projectId.add(1));
