...
export default async function suite() {
  describe('Check execute lottery', async () => {
    it('when execute lottery with a wrong id ', async function () {
      await expectRevert(
        this.investmentContract.executeLotteryRun(1000),
    ...
    it('when execute show interest for investment with wrong investmentId param ', async function () {
      await expectRevert(
        this.investmentContract.showInterestForInvestment(10, 20),
    ...
    it('when execute show interest for investment with wrong partitions ', async function () {
      await expectRevert(
        this.investmentContract.showInterestForInvestment(this.projectId, 0),
    ...
    it('when execute show interest for investment without lending tokens should revert', async function () {
      await expectRevert(
        this.investmentContract.showInterestForInvestment(this.projectId, 10),
    ...
    it('when execute lottery and the state is not started should revert  ', async function () {
      ...
      await expect(
        ...
        .to.emit(this.investmentContract, 'ProjectInterest')
        .withArgs(this.projectId, numberOfPartitions);

      // Then
      await expectRevert(
        this.investmentContract.executeLotteryRun(this.projectId),
        'Can run lottery only in Started state'
      );
    });
    ...
    it('when try to convert nft and the state is not settled should revert  ', async function () {
      ...
      // Then
      await expectRevert(
        this.investmentContract.convertNFTToInvestmentTokens(this.projectId, 10),
    ...
    it('Cannot withdraw tickets in a non settled state', async function () {
      await expectRevert(
        this.investmentContract.convertInvestmentTicketsToNfts(
          this.projectId,
        ),
    ...
    it('Withdraw amount provided for non won tickets', async function () {
      ...
      const baseAmountForEachPartition = await this.investmentContract.baseAmountForEachPartition();
    ...
    it('When executing multiple runs of the lottery', async function () {
      ...
      const statusBeforeSecondRun = await this.investmentContract.projectStatus(this.projectId.add(1));

      await this.investmentContract
        .connect(this.lender2Signer)
        .executeLotteryRun(this.projectId.add(1));

      const statusAfterSecondRun = await this.investmentContract.projectStatus(this.projectId.add(1));
    ...
    context('When doing various withdrawals scenarios', async function () {
      ...
      it('When withdrawing with 0 ticketsToWithdraw', async function () {
        ...
        const lockedNftsBefore = await this.investmentContract.lockedNftsPerAddress(this.lender1);
        ...
        const lockedNftsAfter = await this.investmentContract.lockedNftsPerAddress(this.lender1);

        const NFTTokensGot = balanceOfNFTTokensAfter.sub(balanceOfNFTTokensBefore);

        // Then
        expect(NFTTokensGot.toString()).to.be.equal('0');
        expect(lockedNftsAfter.sub(lockedNftsBefore).toString()).to.be.equal(ticketsToLock.toString());

        await expectRevert(this.investmentContract
            .connect(this.lender1Signer)
      ...
      it('When withdrawing only non-won tickets', async function () {
        ...
        const baseAmountForEachTicket = await this.investmentContract.baseAmountForEachPartition();
    ...
    context('When doing various withdrawals scenarios after already locked tokens', async function () {
      beforeEach(async function () {
        ...
        await this.investmentContract
          .connect(this.lender1Signer)
          .convertInvestmentTicketsToNfts(this.projectId.add(1));
      ...
      it('When showing interest should have more rALBT', async function () {
        ...
        await this.investmentContract
          .connect(this.lender1Signer)
          .showInterestForInvestment(this.projectId.add(2), numberOfPartitions);
      ...
      it('Should revert when trying to withdraw more than locked tickets', async function () {
        ...
        await expectRevert(this.investmentContract
          .connect(this.lender1Signer)
          .withdrawLockedInvestmentNfts(this.projectId.add(1), numberOfPartitions),
      ...
      it('Should revert when trying to withdraw locked tickets on non settled state', async function () {
        ...
        await expectRevert(this.investmentContract
          .connect(this.lender1Signer)
          .withdrawLockedInvestmentNfts(this.projectId, numberOfPartitions),
          'Can withdraw only in Settled state');
      });
    });
  });
