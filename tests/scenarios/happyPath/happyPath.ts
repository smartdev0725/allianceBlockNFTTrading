...
export default async function suite() {
  it('should do a full flow', async function () {
    ...
    expect(Number(stakingContractALBTBalanceAfter)).to.be.equal(
      Number(stakingContractALBTBalanceBefore.add(amountToStake1))
    ...
    expect(Number(stakedSupplyAfter)).to.be.equal(
      Number(stakedSupplyBefore.add(amountToStake1))
    ...
    console.log(`(rALBTBalanceBefore)`, parseFloat(rALBTBalanceBefore))
    console.log(`(rALBTBalanceAfter)`, parseFloat(rALBTBalanceAfter))
    // await this.registryContract
