...
contract ProjectLoan is LoanDetails {
    ...
    // Events
    event ProjectLoanRequested(uint256 indexed loanId, address indexed user, uint256 amount);
    event ProjectLoanMilestoneApprovalRequested(uint256 indexed loanId, uint256 milestoneNumber);
    ...
    function requestProjectLoan(
        ...
        require(
            (amountRequestedPerMilestone.length == totalMilestones) && (milestoneDurations.length == totalMilestones),
            "Total milestones requested should coincide with requested amounts and durations"
        );

        uint256 totalAmountRequested =
            _storeMilestoneDetailsAndGetTotalAmount(amountRequestedPerMilestone, milestoneDurations, totalMilestones);
        ...
        _storeProjectLoanPayments(discountPerMillion, projectTokenPrice, totalMilestones, paymentTimeInterval);

        IERC20(collateralToken).transferFrom(msg.sender, address(escrow), collateralAmount);
    ...
    function _storeProjectLoanPayments(
        ...
        projectLoanPayments[totalLoans].discountPerMillion = discountPerMillion_;
        projectLoanPayments[totalLoans].milestoneProjectTokenPrice[0] = projectTokenPrice_;
    ...
    function _storeMilestoneDetailsAndGetTotalAmount(
        ...
        for (uint256 i = 0; i < totalMilestones; i++) {
            require(
                amountRequestedPerMilestone[i].mod(baseAmountForEachPartition) == 0,
    ...
    function _startProjectLoan(uint256 loanId_) internal {
        ...
        if (
            projectLoanPayments[loanId_].totalMilestones == 1 && projectLoanPayments[loanId_].milestoneDuration[0] == 0
        ) {
            _approveMilestone(loanId_);
        }

        escrow.transferLendingToken(loanSeeker[loanId_], projectLoanPayments[loanId_].milestoneLendingAmount[0]);
    }
     ...
     */
    function _getMockedPriceForMilestone(uint256 milestone) internal returns (uint256 price) {
    ...
    function _challengeProjectLoan(uint256 loanId_) internal {
        ...
        if (projectLoanPayments[loanId_].milestonesExtended > 1) {
            ...
            projectLoanPayments[loanId_].currentMilestoneDeadlineTimestamp = projectLoanPayments[loanId_]
    ...
    function _transferFundingNFTToProjectFunder(
        ...
        uint256 tokenGeneration = projectLoanPayments[loanId_].totalMilestones.sub(1);
        ...
        for (uint256 i = 0; i < projectLoanPayments[loanId_].totalMilestones.sub(1); i++) {
            uint256 partitionsToConvertAtMilestone =
                partitionsFunded_.mul(projectLoanPayments[loanId_].milestoneLendingAmount[i]).div(
                    loanDetails[loanId_].lendingAmount
                );

            fundingNFT.decreaseGenerations(tokenId, funder_, partitionsToConvertAtMilestone, tokenGeneration.sub(i));
        }
    }
     ...
     * @param loanId_ the id of the loan
     */
    function _executeProjectLoanPayment(uint256 loanId_) internal onlyOnProjectRepayment(loanId_) {
        IERC20(lendingToken).transferFrom(msg.sender, address(escrow), getAmountToBeRepaid(loanId_));
    ...
    function _receiveLendingTokenPayment(
        ...
        require(fundingNFT.balanceOf(msg.sender, tokenId) >= amountOfTokens_, "Insufficient Loan NFT Balance");
        uint256 amountToReceive =
            getAmountToBeRepaid(loanId_).mul(amountOfTokens_).div(
                loanDetails[loanId_].totalPartitions.sub(projectLoanPayments[loanId_].partitionsPaidInProjectTokens)
            );
     ...
     */
    function _receiveProjectTokenPayment(uint256 loanId_, uint256 amountFundingNFT_) internal {
        require(
            getAvailableFundingNFTForConversion(loanId_, msg.sender) >= amountFundingNFT_,
            "No funding NFT available for conversion to project tokens"
        );

        uint256 amount = getAmountOfProjectTokensToReceive(loanId_, amountFundingNFT_);

        // Keep track of the partitions paid in project tokens to reduce them from the settlement amount after milestone delivery
        projectLoanPayments[loanId_].partitionsPaidInProjectTokens = projectLoanPayments[loanId_]
        ...
        escrow.transferCollateralToken(loanDetails[loanId_].collateralToken, msg.sender, amount);

        emit ProjectTokenPaymentReceived(loanId_, msg.sender, amount, getDiscountedProjectTokenPrice(loanId_));
    }
     ...
     */
    function _burnFundingNFTAmountOverGenerations(uint256 loanId_, uint256 amountFundingNFT_) internal {
        uint256 totalFundingNFTToBurn = amountFundingNFT_;
        for (uint256 i = 0; i < projectLoanPayments[loanId_].milestonesDelivered && totalFundingNFTToBurn > 0; i++) {
            uint256 fundingNFTBalance = balanceOfFundingNFTGeneration(loanId_, i, msg.sender);
            uint256 fundingNFTToBurn =
                fundingNFTBalance > totalFundingNFTToBurn ? totalFundingNFTToBurn : fundingNFTBalance;
     ...
     */
    function getAmountToBeRepaid(uint256 loanId) public view returns (uint256 amount) {
        ...
        uint256 lendingTokenAmount =
            loanDetails[loanId].lendingAmount.sub(
                projectLoanPayments[loanId].partitionsPaidInProjectTokens.mul(baseAmountForEachPartition)
            );
        // Calculate the interest only over what is left to pay in the lending token
        uint256 interest = lendingTokenAmount.mul(loanDetails[loanId].interestPercentage).div(100);
     ...
     */
    function getTotalInterest(uint256 loanId) public view returns (uint256 totalInterest) {
        // Substract the partitions already paid in project tokens from the lending amount to pay back
        uint256 lendingTokenAmount =
            loanDetails[loanId].lendingAmount.sub(
                projectLoanPayments[loanId].partitionsPaidInProjectTokens.mul(baseAmountForEachPartition)
        ...
        // Calculate the interest only over what is left to pay in the lending token
        totalInterest = lendingTokenAmount.mul(loanDetails[loanId].interestPercentage).div(100);
    }
     ...
     */
    function balanceOfAllFundingNFTGenerations(uint256 loanId, address funder) public view returns (uint256 balance) {
        for (uint256 i = 0; i < projectLoanPayments[loanId].totalMilestones; i++) {
            balance = balance.add(balanceOfFundingNFTGeneration(loanId, i, funder));
        }
    }
     ...
     */
    function getProjectTokenPrice(uint256 loanId) public view returns (uint256 price) {
        ...
        uint256 milestonePriceWasSet =
            projectLoanPayments[loanId].milestonesDelivered > 0
                ...
                : 0;
        price = projectLoanPayments[loanId].milestoneProjectTokenPrice[milestonePriceWasSet];
    }
     ...
     */
    function getDiscountedProjectTokenPrice(uint256 loanId) public view returns (uint256 price) {
        uint256 marketPrice = getProjectTokenPrice(loanId);
        price = marketPrice.sub(marketPrice.mul(projectLoanPayments[loanId].discountPerMillion).div(10**6));
    }
     ...
     */
    function getAvailableFundingNFTForConversion(uint256 loanId, address funder) public view returns (uint256 balance) {
        ...
        for (uint256 i = 0; i < projectLoanPayments[loanId].milestonesDelivered; i++) {
            balance = balance.add(balanceOfFundingNFTGeneration(loanId, i, funder));
        }
    }
     ...
     */
    function getAmountOfProjectTokensToReceive(uint256 loanId, uint256 amountFundingNFT)
        public
        view
        returns (uint256 amount)
    {
        uint256 amountToReceiveInProjectTokens = amountFundingNFT.mul(baseAmountForEachPartition);
        // Calculate amount of project tokens based on the discounted price
        amount = amountToReceiveInProjectTokens.div(getDiscountedProjectTokenPrice(loanId));
    }
}
