...
contract PersonalLoan is LoanDetails {
    ...
    // Events
    event PersonalLoanRequested(uint256 indexed loanId, address indexed user, uint256 amount);
    ...
    function requestPersonalLoan(
        ...
        _storePersonalLoanPayments(totalAmountOfBatches, batchTimeInterval, repaymentBatchType);
    ...
    function _storePersonalLoanPayments(
        ...
        if (repaymentBatchType_ == LoanLibrary.RepaymentBatchType.ONLY_INTEREST) {
            ...
            personalLoanPayments[totalLoans].amountEachBatch = (
                loanDetails[totalLoans].totalInterest.add(loanDetails[totalLoans].lendingAmount)
            )
    ...
    function _startPersonalLoan(uint256 loanId_) internal {
        ...
        escrow.transferLendingToken(loanSeeker[loanId_], loanDetails[loanId_].lendingAmount);
    }
    ...
    function _challengePersonalLoan(uint256 loanId_) internal {
        ...
        if (personalLoanPayments[loanId_].batchesSkipped > 1) {
            ...
            personalLoanPayments[loanId_].batchDeadlineTimestamp = personalLoanPayments[loanId_]
     ...
     */
    function _transferPersonalLoanPayment(uint256 loanId_, uint256 amount) internal {
    ...
    function _receivePersonalLoanPayment(
        ...
        uint256 amountToBePaid =
            personalLoanPayments[loanId_].amountEachBatch.mul(amountOfTokens_).mul(batchesToBePaid).div(
        ...
        if (loanStatus[loanId_] == LoanLibrary.LoanStatus.SETTLED) {
            if (personalLoanPayments[loanId_].repaymentBatchType == LoanLibrary.RepaymentBatchType.ONLY_INTEREST) {
                amountToBePaid = amountToBePaid.add(
                    loanDetails[loanId_].lendingAmount.mul(amountOfTokens_).div(loanDetails[loanId_].totalPartitions)
                );
            }

            fundingNFT.burn(msg.sender, generation_.getTokenId(loanId_), amountOfTokens_);
        } else {
