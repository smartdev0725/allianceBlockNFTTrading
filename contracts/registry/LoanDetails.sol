...
contract LoanDetails is Storage {
    ...
    modifier onlySeeker(uint256 loanId) {
        require(msg.sender == loanSeeker[loanId], "Only seeker of the loan");
        _;
    }
    ...
    modifier onlyActivelyFundedLoan(uint256 loanId) {
        ...
        require(
            loanDetails[loanId].approvalDate.add(fundingTimeInterval) > block.timestamp,
    ...
    modifier onlyActiveLoan(uint256 loanId) {
        require(
            loanStatus[loanId] == LoanLibrary.LoanStatus.STARTED ||
                loanStatus[loanId] == LoanLibrary.LoanStatus.AWAITING_MILESTONE_APPLICATION,
    ...
    modifier onlyOnProjectRepayment(uint256 loanId) {
        require(loanStatus[loanId] == LoanLibrary.LoanStatus.AWAITING_REPAYMENT, "Only on Repayment Status");
        require(
            projectLoanPayments[loanId].awaitingForRepaymentDate.add(projectLoanPayments[loanId].paymentTimeInterval) >
                block.timestamp,
    ...
    modifier onlySettledLoan(uint256 loanId) {
        require(loanStatus[loanId] == LoanLibrary.LoanStatus.SETTLED, "Only on Settled Status");
        _;
    }

    modifier onlyBetweenMilestoneTimeframe(uint256 loanId) {
        require(
            projectLoanPayments[loanId].currentMilestoneDeadlineTimestamp > block.timestamp &&
    ...
    modifier onlyBetweenBatchTimeframe(uint256 loanId) {
        require(
            personalLoanPayments[loanId].batchDeadlineTimestamp > block.timestamp &&
    ...
    modifier onlyPersonalLoan(uint256 loanId) {
        require(loanDetails[loanId].loanType == LoanLibrary.LoanType.PERSONAL, "Only when loan is personal");
        _;
    }

    modifier onlyProjectLoan(uint256 loanId) {
        require(loanDetails[loanId].loanType == LoanLibrary.LoanType.PROJECT, "Only when loan is for project");
        _;
    }
    ...
    modifier onlyEnoughERC1155Balance(uint256 loanId, uint256 amountOfTokens) {
        ...
        for (uint256 i = 0; i < projectLoanPayments[loanId].totalMilestones; i++) {
            balance = balance.add(fundingNFT.balanceOf(msg.sender, i.getTokenId(loanId)));
        }
